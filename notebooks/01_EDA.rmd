...
```
# Addition_1:
``` {r Quarter seasonality check for household net savings}
eda_df <- eda_df |>
  mutate(
    quarter_lab = factor(quarter(date), labels = c("Q1", "Q2", "Q3", "Q4"))
  )

ggplot(eda_df, aes(x = quarter_lab, y = hh_save)) +
  geom_boxplot(fill = "steelblue", alpha = 0.7) +
  theme_minimal() +
  labs(
    title = "Household net savings by quarter",
    x = "Quarter",
    y = "Net savings"
  )

quarter_summary <- eda_df |>
  group_by(quarter_lab) |>
  summarise(
    mean_hh_save = mean(hh_save, na.rm = TRUE),
    sd_hh_save = sd(hh_save, na.rm = TRUE),
    .groups = "drop"
  )

kable(quarter_summary, digits = 1,
      caption = "Quarterly summary of household net savings")

```

# Addition_2:
```{r Quarterly seasonality and STL decomposition for household net savings}


hh_ts <- ts(eda_df$hh_save, start = c(1977, 1), frequency = 4)

hh_stl <- stl(hh_ts, s.window = "periodic", robust = TRUE)

plot(hh_stl, main = "STL decomposition of household net savings")

stl_comp <- hh_stl$time.series

seasonal_strength <- max(
  0,
  1 - var(stl_comp[, "remainder"], na.rm = TRUE) /
    var(stl_comp[, "seasonal"] + stl_comp[, "remainder"], na.rm = TRUE)
)

trend_strength <- max(
  0,
  1 - var(stl_comp[, "remainder"], na.rm = TRUE) /
    var(stl_comp[, "trend"] + stl_comp[, "remainder"], na.rm = TRUE)
)

stl_strength_tbl <- tibble(
  component = c("seasonal_strength", "trend_strength"),
  value = c(seasonal_strength, trend_strength)
)

kable(stl_strength_tbl, digits = 3, caption = "STL component strength for household net savings")
```
The quarterly boxplot suggests only weak seasonal differences across Q1–Q4. This is consistent with the STL result, although STL extracts a periodic seasonal component, its estimated seasonal strength is essentially zero, indicating that seasonal variation is very small relative to the irregular component.
...
## STL Decomposition of Household Net Savings

We apply STL (Seasonal and Trend decomposition using Loess) to the level `hh_save` series to separate trend, seasonal, and remainder components. This helps identify whether any quarterly seasonal pattern exists in household savings and how much of the variation is driven by irregular shocks versus systematic dynamics.

```{r STL decomposition, fig.width=10, fig.height=8}
stl_hh <- stl(hh_ts, s.window = "periodic")

plot(stl_hh, main = "STL Decomposition: Household Net Savings (Level)")
```

```{r STL component magnitudes}
stl_comp <- stl_hh$time.series
cat("Component standard deviations:\n")
cat("  Trend:     ", round(sd(stl_comp[, "trend"]), 1), "\n")
cat("  Seasonal:  ", round(sd(stl_comp[, "seasonal"]), 1), "\n")
cat("  Remainder: ", round(sd(stl_comp[, "remainder"]), 1), "\n")
cat("\nRatio of remainder SD to total SD:", 
    round(sd(stl_comp[, "remainder"]) / sd(hh_ts), 3), "\n")
```

**Interpretation:**

- The **trend** component captures the long-run rise in household savings through the late 1980s, the sharp decline through the 1990s and 2000s, and the partial recovery post-2008. These movements align with Canadian macroeconomic cycles (oil booms, rate hikes, the 2008 crisis).
- The **seasonal** component is modest relative to the trend and remainder; quarterly seasonality in savings is present but small. This explains why SARIMA models offer only marginal improvement over non-seasonal ARIMA.
- The **remainder** (irregular) component dominates the short-term fluctuations. Its large variance relative to the seasonal component confirms that household savings are largely driven by unpredictable quarter-to-quarter shocks, consistent with the near-random-walk behaviour observed in the ACF analysis.
...
#Addition_3
```{r Compare sample ACF and variance for stationary}
compare_sample_acf_var <- function(y, lag.max = 12, split_frac = 0.5, series_name = "series") {
  y <- as.numeric(na.omit(y))
  
  n_full <- length(y)
  n_short <- floor(split_frac * n_full)
  
  y_short <- y[1:n_short]
  y_full  <- y
  
  par(mfrow = c(2, 2))
  plot.ts(y_short, main = paste0(series_name, ": first ", n_short, " observations"))
  plot.ts(y_full,  main = paste0(series_name, ": full ", n_full, " observations"))
  acf(y_short, lag.max = lag.max, main = paste0("ACF: first ", n_short))
  acf(y_full,  lag.max = lag.max, main = paste0("ACF: full ", n_full))
  par(mfrow = c(1, 1))
  
  out_tbl <- tibble(
    sample = c(paste0("first_", n_short), paste0("full_", n_full)),
    mean = c(mean(y_short), mean(y_full)),
    variance = c(var(y_short), var(y_full)),
    sd = c(sd(y_short), sd(y_full))
  )
  
  kable(out_tbl, digits = 3,
        caption = paste0(series_name, ": mean and variance comparison across sample lengths"))
  
  invisible(out_tbl)
}

# Level series
level_compare_tbl <- compare_sample_acf_var(
  hh_ts,
  lag.max = 12,
  split_frac = 0.5,
  series_name = "Household net savings"
)

# Differenced series
diff_compare_tbl <- compare_sample_acf_var(
  dhh_ts,
  lag.max = 12,
  split_frac = 0.5,
  series_name = "First difference of household net savings"
)
```
Comparing the first half of the sample with the full sample shows that household net savings in levels is not stable across sample lengths. Its ACF remains large and slowly decays, which is consistent with non-stationarity. After first differencing, the series fluctuates around a more stable level and the ACF becomes much smaller, supporting the use of differencing before ARIMA-type modeling.
